...
! convert x y z to r theta phi, single precision call
  ...
  theta = real(datan2(dsqrt(xmesh*xmesh+ymesh*ymesh),zmesh), kind=8)

  if (xmesh > -SMALL_VAL_ANGLE .and. xmesh <= ZERO) xmesh = -SMALL_VAL_ANGLE
  if (xmesh < SMALL_VAL_ANGLE .and. xmesh >= ZERO) xmesh = SMALL_VAL_ANGLE

  phi = real(datan2(ymesh,xmesh), kind=8)

  r = real(dsqrt(xmesh*xmesh + ymesh*ymesh + zmesh*zmesh), kind=8)
...
! gives some thoughts.
  ...
  use constants, only: PI_OVER_TWO,TINYVAL,ASSUME_PERFECT_SPHERE, &
      USE_OLD_VERSION_5_1_5_FORMAT, ONE_MINUS_F_SQUARED
...
! converts geocentric colatitude (theta) to geographic colatitude (theta_prime) (in radians)
  ...
  theta_prime = real(dtheta_prime, kind=8)
