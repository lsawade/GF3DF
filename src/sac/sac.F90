...
contains
  ...
  ! SAC header file format: https://ds.iris.edu/files/sac-manual/manual/file_format.html
    ...
    ! Based on the IRIS convention, S1 and S3 are assigned to 1D and 3D seismogram, respectively.
    ...
    KUSER0 = 'SY'          ! Network code assigned by IRIS for synthetic seismogram
    ...
    if (OUTPUT_SEISMOS_SAC_ALPHANUM) then
      ...
      ! add .sacan (sac alphanumeric) extension to seismogram file name for SAC seismogram
  ...
  540 format(A8,A8,A8)
      ...
      if (imodulo_5 == 0) then
        ...
        do isample = 1+5,NT+1,5
          value1 = dble(seismogram(isample-5))
          value2 = dble(seismogram(isample-4))
          value3 = dble(seismogram(isample-3))
          value4 = dble(seismogram(isample-2))
          value5 = dble(seismogram(isample-1))
        ...
        if (imodulo_5 >= 1) value1 = dble(seismogram(NT-imodulo_5+1))
        if (imodulo_5 >= 2) value2 = dble(seismogram(NT-imodulo_5+2))
        if (imodulo_5 >= 3) value3 = dble(seismogram(NT-imodulo_5+3))
        if (imodulo_5 >= 4) value4 = dble(seismogram(NT-imodulo_5+4))
    ...
    if (OUTPUT_SEISMOS_SAC_BINARY) then
      ...
      ! add .sac (sac binary) extension to seismogram file name for SAC seismogram
